...
import 'package:grocbay_pos/feautures/grocbay_pos/domain/usecases/get_category_product.dart';
  ...
  ProductBloc(GetCategoryProduct getCategoryProduct) : super(ProductStateInitial()) {
    on<ProductEvent>((event, emit) async{
      if(event is OnProductGet) {
        ...
        final value = await getCategoryProduct(data: CatProductParms(id: (event.props.first as FetchSubCategory).id!,start:event.props[1].toString(), end:event.props[2].toString(), branch:"${sl<SharedPreferences>().getString(Prefrence.BRANCH)}", language_id:"0", type: event.props.last as int));
        ...
        emit(ProductStateLoading());
        final value = await getCategoryProduct(data: CatProductParms(id: (event.props.first as FetchSubCategory).id!,start:"0", end:"0", branch:sl<SharedPreferences>().getString(Prefrence.BRANCH)!, language_id:"0", type: event.props.last as int));
