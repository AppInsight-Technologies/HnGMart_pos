...
class SearchProductUsecase extends UseCase< List<FetchCategoryProduct>,ProductsearchData> {
  ...
  Future<Either<Failure,  List<FetchCategoryProduct>>> call({ProductsearchData? data}) async{
    ...
    final value = await repo.getRequest(Params(uri: Uri.parse("v3/restaurant/search-items-by-cart"),methed: Methed.Post,data: {"apiKey":sl<SharedPreferences>().getString(Prefrence.BRANCH),"item_name":data!.query, "branch":sl<SharedPreferences>().getString(Prefrence.BRANCH),"possellingtype":"1" }));
