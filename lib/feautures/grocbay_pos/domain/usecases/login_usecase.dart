...
import 'package:grocbay_pos/core/data/hive_db.dart';
...
class LoginUseCase extends UseCase<getUser,Param> {
  ...
  Future<Either<Failure, getUser>> call({Param? data}) async{
    // TODO: implement call
    print("login");
    final value = await repo.getRequest(Params(uri: Uri.parse("pos-login"),methed: Methed.Post,data: {"email":((data!.props.first as Map<String,dynamic>)["email"]), "password":(data.props.first as Map<String,dynamic>)["password"]}));
    print("hjghjghjg");
