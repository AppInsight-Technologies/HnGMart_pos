...
import 'package:grocbay_pos/feautures/grocbay_pos/domain/entities/cart_item_modle.dart';
import 'package:grocbay_pos/feautures/grocbay_pos/domain/entities/get_profile_modle.dart';
...
class UsbPrinter{
 ...
 const UsbPrinter(this.flutterUsbPrinter);
  Future<List<int>> _testTicket(OrderItem odata, List<CartItemModel> list_cart_items, GetCustomerProfile user,
      String waletamt, String? restaurantName, String? restaurantAddress,String? primaryEmail,String? primaryMobile) async{
    // final List<int> bytes = [];
    // Using default profile
 //   debugPrint("_testTicket..."+restaurantName.toString()+".."+restaurantAddress.toString()+".."+primaryEmail.toString()+".."+primaryMobile.toString());
    ...
    bytes += generator.text(restaurantAddress!, styles: const PosStyles(align: PosAlign.center));
    bytes += generator.text("GST NO: -",styles: const PosStyles(align: PosAlign.center));
    bytes += generator.text("Mobile: "+primaryMobile!,styles: const PosStyles(align: PosAlign.center));
    bytes += generator.text("Email : "+primaryEmail!,styles: const PosStyles(align: PosAlign.center), linesAfter: 2);
 ...
 //   bytes += generator.text('Slot: 2020',);
   // bytes += generator.text('Order Type : POS', linesAfter: 1);
    ...
    bytes += generator.text("thank you for shopping with "+restaurantName,styles: const PosStyles(align: PosAlign.center));
  ...
  print(OrderItem odata, List<CartItemModel> list_cart_items, GetCustomerProfile user, String waletamt, String? restaurantName, String? restaurantAddress, String? primaryEmail, String? primaryMobile) async {
    try {
      var bytes = await _testTicket(odata,list_cart_items,user,waletamt,restaurantName,restaurantAddress,primaryEmail,primaryMobile);
